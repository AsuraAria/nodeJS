...
const data = require("../app_resources/data.json");
...
class Todos {
  ...
  get size() { return this.todos.size; }
  ...
  getTodos() {
    let tabTodos = [];
    for (const v of this.todos.values()) { tabTodos.push(v); }
    return tabTodos;
  }
  deleteTodos() { this.todos.clear(); }

}

function logMapElements(value, key, map) { console.log("m[" + key + "] = " + JSON.stringify(value)); }
